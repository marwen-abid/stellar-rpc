...
func newQueryRequest(
	...
	case queryTypeEvents:
		...
		if p.Extra != nil {
			p.Extra["eventsCorpus"] = corpusKind(len(corpus.sets))
		}
...
func ledgersRequest(f *queryFixture, p queryPlan) queryRequest {
	return func(rng *rand.Rand) (cellSample, error) {
		lo, hi := f.pickRange(rng, p.LedgersSpan)
...
func txPageRequest(f *queryFixture, p queryPlan) queryRequest {
	return func(rng *rand.Rand) (cellSample, error) {
		lo, hi := f.pickRange(rng, p.TxPageSpan)
		return timed("", func() (int, error) {
			view, err := f.view()
			if err != nil {
				return 0, fmt.Errorf("acquire read view: %w", err)
...
// pickRange returns a random span-long ledger range inside the fixture's
// range. The end is clamped to the fixture's last ledger, which a hot fixture
// sets below the registry's latest when --sample-ledgers narrows the window.
func (f *queryFixture) pickRange(rng *rand.Rand, span uint32) (uint32, uint32) {
	lo := f.pickStart(rng, span)
	return lo, min(lo+span-1, f.LastLedger)
}
