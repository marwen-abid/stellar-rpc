...
func TestQueryCommandPersistsFailedMeasurements(t *testing.T) {
	...
	assertLegDriverRows(t, driver, queryTypeTxPage, 100, 4)
	assert.EqualValues(t, 4, driver["txpage_r100_dispatched"]["n_items"])
	assert.EqualValues(t, 4, driver["txpage_r100_failed"]["n_items"])
	assert.Zero(t, driver["txpage_r100_successful"]["n_items"])
	assert.Zero(t, driver["txpage_r100_shed"]["n_items"])
	assert.Zero(t, driver["txpage_r100_millirps"]["total_ns"])
	assert.Zero(t, driver["txpage_r100_completion_millirps"]["total_ns"])
	assert.NotContains(t, driver, "txpage_r200_scheduled", "failure stops later legs")
