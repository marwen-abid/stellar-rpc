...
func assertRequiredFlags(t *testing.T, cmd *cobra.Command, requiredBySubcommand map[string][]string) {
	t.Helper()
	subs := subcommandsByName(cmd)
	require.Len(t, subs, len(requiredBySubcommand), "every subcommand needs its required flags checked")
...
// subcommandsByName indexes a command's children by the name cobra routes on.
func subcommandsByName(cmd *cobra.Command) map[string]*cobra.Command {
...
// coldTxEvery: every coldTxEvery-th ledger of the cold fixture carries
// transactions, dense enough that the bounded sampler always reaches two of them.
const coldTxEvery = 10

// testCorpusSize is --txhash-corpus-size for these runs: one whole per-ledger
// draw plus one more hash, so the corpus must reach into a second ledger. The
// cold fixture puts exactly that many transactions in each tx-bearing ledger.
const testCorpusSize = corpusMaxHashesPerLedger + 1

// scratchQueryCatalogs lists the scratch query catalog dirs under dir.
func scratchQueryCatalogs(t *testing.T, dir string) []string {
	t.Helper()
	matches, err := filepath.Glob(filepath.Join(dir, scratchPrefixQuery+"*"))
...
func TestQueryCommandRecordsCacheAndCorpus(t *testing.T) {
	...
	packDir := writeLedgerPack(t, t.TempDir(), chunk.ID(0), hotLedgers, func(seq uint32) []byte {
		return rpcv2test.MultiTxLCMBytes(t, seq, testCorpusSize)
	})
	hotRoot := ingestHotChunk(t, packDir, hotLedgers)
	// Cold opening needs a complete chunk, but only some of its ledgers need transactions.
	coldPack := writeLedgerPack(t, t.TempDir(), chunk.ID(0), chunk.LedgersPerChunk, func(seq uint32) []byte {
		if (seq-chunk.ID(0).FirstLedger())%coldTxEvery == 0 {
			return rpcv2test.MultiTxLCMBytes(t, seq, testCorpusSize)
		}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			out := t.TempDir()
			args := []string{
				tc.tier, "--out=" + out, "--types=txhash", "--txhash-corpus-size=" + strconv.Itoa(testCorpusSize),
				"--target-rps=100", "--duration=40ms", "--miss-fraction=0",
			}
			// The scratch catalog goes under --catalog-dir, so the run leaves
			// no scratch catalog in the dataset root it measures. The base dir
			// is created on demand, and the catalog under it is removed at the
			// end.
			catalogDir := filepath.Join(t.TempDir(), "catalogs")
			datasetRoot := coldRoot
			if tc.tier == "hot" {
				datasetRoot = hotRoot
			...
			args = append(args, "--catalog-dir="+catalogDir)
			...
			require.DirExists(t, catalogDir, "the run created the catalog base it was given")
			assert.Empty(t, scratchQueryCatalogs(t, datasetRoot), "the dataset root holds no scratch catalog")
			assert.Empty(t, scratchQueryCatalogs(t, catalogDir), "the run removed the catalog it created")
			...
			assert.Equal(t, strconv.Itoa(testCorpusSize), record.Flags["txhash-corpus-size"])
			assert.Equal(t, tc.scenario, record.Extra["cacheScenario"])
			assert.Equal(t, tc.eviction, record.Extra["pageCacheEviction"])
			assert.Equal(t, strconv.Itoa(testCorpusSize), record.Extra["txhashCorpusHashes"], "the requested cap is exact")
			assert.Equal(t, "2", record.Extra["txhashCorpusLedgers"],
				"a full per-ledger draw plus one hash from a second ledger")
...
func TestQueryCommandPersistsFailedMeasurements(t *testing.T) {
	...
	cmd.SetOut(io.Discard)
	cmd.SetErr(io.Discard)
	// The fixture is hashed under pubnet, so a testnet passphrase makes every
	// txpage envelope pairing fail; events and ledgers do not pair and still complete.
	cmd.SetArgs([]string{
