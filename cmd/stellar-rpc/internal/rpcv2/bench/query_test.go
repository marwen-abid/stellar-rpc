...
func TestQueryCommandAcceptsRunnerArgv(t *testing.T) {
	for _, tc := range []struct {
		name string
		argv []string
		// wantSampleLedgers: what --sample-ledgers parses to, zero when the
		// argv leaves the sampled range uncapped.
		wantSampleLedgers uint32
	}{
		{
			name: "cold",
			argv: []string{
		...
		},
		{
			...
			wantSampleLedgers: 50000,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			// ParseFlags skips cobra's required-flag and positional checks,
			// so the argv must reach a run to pin the whole contract.
			root := NewQueryCommand()
			var sub *cobra.Command
			for _, c := range root.Commands() {
				c.RunE = func(cmd *cobra.Command, _ []string) error { sub = cmd; return nil }
			}
			root.SetOut(io.Discard)
			root.SetErr(io.Discard)
			root.SetArgs(tc.argv)
			require.NoError(t, root.Execute(), "the runner's argv must reach a run")
			require.NotNil(t, sub)
			require.Equal(t, tc.argv[0], sub.Name())
			...
			// The tier-specific flags are read back by type, so a flag that
			// silently changes type fails here as well as a renamed one.
			out, err := sub.Flags().GetString("out")
			require.NoError(t, err)
			require.Equal(t, "/bench/out", out)

			plan, err := planFromArgv(t, sub)
			require.NoError(t, err, "the runner's argv must form a valid plan")
			assert.Equal(t, warmup, plan.Warmup, "the plan carries the tier's warmup")

			assertTierFlags(t, sub, tc.wantSampleLedgers)
		})
	}
}

// assertTierFlags reads back the flags one tier binds: the cold chunk range, or
// the hot chunk and its sampled-ledger cap.
func assertTierFlags(t *testing.T, sub *cobra.Command, wantSampleLedgers uint32) {
	t.Helper()
	if sub.Name() == "cold" {
		startChunk, err := sub.Flags().GetUint32("start-chunk")
		require.NoError(t, err)
		assert.EqualValues(t, 7, startChunk)
		numChunks, err := sub.Flags().GetInt("num-chunks")
		require.NoError(t, err)
		assert.Equal(t, 1, numChunks)
		coldDir, err := sub.Flags().GetString("cold-dir")
		require.NoError(t, err)
		assert.Equal(t, "/bench/ds", coldDir)
		return
	}
	chunkID, err := sub.Flags().GetUint32("chunk")
	require.NoError(t, err)
	assert.EqualValues(t, 7, chunkID)
	hotDir, err := sub.Flags().GetString("hot-dir")
	require.NoError(t, err)
	assert.Equal(t, "/bench/hot", hotDir)
	sampleLedgers, err := sub.Flags().GetUint32("sample-ledgers")
	require.NoError(t, err)
	assert.Equal(t, wantSampleLedgers, sampleLedgers)
}

// planFromArgv builds the plan the parsed argv implies. plan() validates beyond
// parsing, and the subcommand's own queryFlags is closed over, so the flags the
// argv set are replayed onto an equivalent binding seeded from the same tier.
// Unset flags keep that tier's defaults.
func planFromArgv(t *testing.T, sub *cobra.Command) (queryPlan, error) {
	t.Helper()
	shadow := &cobra.Command{Use: "shadow"}
	qf := queryTierFlags(sub.Name())
	qf.bind(shadow)
	sub.Flags().Visit(func(f *pflag.Flag) {
		if shadow.Flags().Lookup(f.Name) == nil {
			return
		}
		require.NoError(t, shadow.Flags().Set(f.Name, f.Value.String()))
	})
	return qf.plan()
}

// TestQueryDefaultShapesAreTheCampaignDefaults pins the read shapes a campaign
// run gets when the runner passes none of these flags.
func TestQueryDefaultShapesAreTheCampaignDefaults(t *testing.T) {
	for _, name := range []string{"cold", "hot"} {
		t.Run(name, func(t *testing.T) {
			sub := subcommandsByName(NewQueryCommand())[name]
			require.NotNil(t, sub)

			wantWarmup := 0
			if name == queryTierHot {
				wantWarmup = defaultHotWarmup
			}
			warmup, err := sub.Flags().GetInt("warmup")
			require.NoError(t, err)
			require.Equal(t, wantWarmup, warmup, "campaign default: only the hot tier pre-warms")

			ledgersSpan, err := sub.Flags().GetUint32("ledgers-span")
			require.NoError(t, err)
			require.Equal(t, uint32(10), ledgersSpan, "campaign default: getLedgers reads 10 ledgers")

			eventsLimit, err := sub.Flags().GetInt("events-limit")
			require.NoError(t, err)
			require.Equal(t, 10, eventsLimit, "campaign default: getEvents pages 10 events")

			txPageSpan, err := sub.Flags().GetUint32("txpage-span")
			require.NoError(t, err)
			require.Equal(t, uint32(5), txPageSpan, "campaign default: getTransactions walks 5 ledgers")

			txPageLimit, err := sub.Flags().GetInt("txpage-limit")
			require.NoError(t, err)
			require.Equal(t, 200, txPageLimit, "campaign default: getTransactions pages 200 transactions")
		})
	}
}
...
// TestQuerySpecs pins the file order and the multi-leg row order of
// query-accounting.csv, which the results converter parses. The per-type,
// driver and single-leg accounting schemas are pinned in csvsink_test.go.
func TestQuerySpecs(t *testing.T) {
	...
	), byName[fileQueryAccounting], "every leg's rows are grouped, in --types then --target-rps order")
}
...
// TestOpenColdFixtureRejectsAChunkMissingItsEventsPack: a chunk with no events
// pack fails at open when --types asks for events, with the chunk named, rather
// than as failed requests once a leg reaches it.
func TestOpenColdFixtureRejectsAChunkMissingItsEventsPack(t *testing.T) {
	chunkID := chunk.ID(0)
	coldRoot := ingestColdChunk(t, chunkID)
	for _, p := range geometry.NewLayout(coldRoot).ArtifactPaths(chunkID, geometry.KindEvents) {
	...
	require.ErrorContains(t, err, "no servable events store")
	assert.ErrorContains(t, err, chunkID.String())
}

// TestOpenColdFixtureReportsAnUnstatableArtifact: an artifact the process
// cannot stat fails the open. Reading it as an absence would either drop the
// kind from the freeze or misreport the dataset as not covering the chunk.
func TestOpenColdFixtureReportsAnUnstatableArtifact(t *testing.T) {
	if os.Geteuid() == 0 {
		t.Skip("root stats a file under a directory with no permissions")
	}
	chunkID := chunk.ID(0)
	coldRoot := ingestColdChunk(t, chunkID)
	dir := filepath.Dir(geometry.NewLayout(coldRoot).LedgerPackPath(chunkID))
	...
	require.ErrorContains(t, err, "stat chunk")
...
func assertLegDriverRows(
	...
	// The elapsed, drain and rate values follow from the emitted numbers;
	// TestRecordQueryLegAccounting pins them against literals.
	for _, column := range []string{"total_ns", "p50_ns", "p90_ns", "p99_ns", "max_ns"} {
		assert.Equal(t, int64(math.Round(rps*1000)), accounting[base+"_target_millirps"][column], column)
	}
}
...
// TestOpenColdFixtureStraddlingTxHashWindows opens a chunk range that crosses a
// tx-hash window boundary. One window index cannot cover the range, so the open
// fails when --types asks for txhash and warns otherwise.
func TestOpenColdFixtureStraddlingTxHashWindows(t *testing.T) {
	lo := chunk.ID(geometry.ChunksPerTxhashIndex - 1) // the last chunk of window 0
	coldRoot := t.TempDir()
	packs := t.TempDir()
	for _, c := range []chunk.ID{lo, lo + 1} {
		packDir, _ := writeSourcePack(t, packs, c, chunk.LedgersPerChunk)
		require.NoError(t, runCold(context.Background(), testLogger(), coldOptions{
			Source:     sourceConfig{Kind: sourcePack, PackDir: packDir},
			StartChunk: c, NumChunks: 1, Workers: 1,
			ColdRoot: coldRoot, OutDir: filepath.Join(t.TempDir(), "csv"),
		}))
	}
	...
	t.Run("txhash requested", func(t *testing.T) {
		...
		require.ErrorContains(t, err, "straddle two tx-hash window indexes")
	})
	...
	t.Run("txhash not requested", func(t *testing.T) {
		...
		require.NoError(t, err)
		defer release()
		assert.Equal(t, []chunk.ID{lo, lo + 1}, f.Chunks)
		assert.Contains(t, strings.Join(logMessages(warnings, logrus.WarnLevel), "\n"),
			"no usable tx-hash window index")
	})
}
...
func TestOpenHotFixtureServesHotChunk(t *testing.T) {
	...
	t.Run("uncapped sample", func(t *testing.T) {
		...
		require.Equal(t, []chunk.ID{chunkID}, f.Chunks)
		assert.Empty(t, f.EvictPaths, "a hot fixture names no page-cache artifact to evict")
...
// TestPickRangeStaysInsideTheFixture covers a span wider than the fixture's
// range, which clamps to the whole range, and a span that fits inside it.
func TestPickRangeStaysInsideTheFixture(t *testing.T) {
	f := &queryFixture{FirstLedger: 100, LastLedger: 109}
	const draws = 32

	t.Run("span wider than the range", func(t *testing.T) {
		rng := testRNG()
		for range draws {
			lo, hi := f.pickRange(rng, 25)
			assert.Equal(t, f.FirstLedger, lo)
			assert.Equal(t, f.LastLedger, hi, "the end never passes the fixture's last ledger")
		}
	})

	t.Run("span inside the range", func(t *testing.T) {
		const span = 4
		rng := testRNG()
		for range draws {
			lo, hi := f.pickRange(rng, span)
			assert.GreaterOrEqual(t, lo, f.FirstLedger)
			assert.LessOrEqual(t, hi, f.LastLedger)
			assert.EqualValues(t, span, hi-lo+1)
		}
	})
}
