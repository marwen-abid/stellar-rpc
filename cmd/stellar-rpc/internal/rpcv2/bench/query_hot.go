...
// queryTierFlags is the flag set a tier's subcommand binds, seeded with the
// defaults bind reads off the struct rather than off its flag definitions.
...
func openHotFixture(logger *supportlog.Entry, opts hotQueryOptions) (*queryFixture, func(), error) {
	...
	cat, releaseCat, err := openScratchCatalog(catalogBase, scratchPrefixQuery, layout, logger)
	if err != nil {
		return nil, nil, err
	}
	...
	if first := opts.Chunk.FirstLedger(); committed < first {
		_ = db.Close()
		releaseCat()
	...
	if err := f.verifyServes(opts.Plan.Types); err != nil {
		release()
		return nil, nil, err
	}
