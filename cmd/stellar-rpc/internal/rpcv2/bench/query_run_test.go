...
func TestRunQueryLegPreservesRequestFailures(t *testing.T) {
	for _, allFailed := range []bool{false, true} {
		t.Run(map[bool]string{false: "partial-success", true: "all-failed"}[allFailed], func(t *testing.T) {
			...
			assert.Contains(t, output.String(), "first error: read failed")
			assert.Contains(t, output.String(), "PARTIAL CSVs")
		})
	}
}

// TestWarnFixedReadRange: a configured span that covers the whole fixture warns
// and lands in the invocation extras; a span that fits inside it stays quiet.
func TestWarnFixedReadRange(t *testing.T) {
	f := &queryFixture{FirstLedger: 10, LastLedger: 14}
	for _, tc := range []struct {
		name        string
		ledgersSpan uint32
		txPageSpan  uint32
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.want == "" {
				...
				assert.NotContains(t, p.Extra, "fixedReadRange")
				return
			}
			assert.Contains(t, output.String(), "fixture holds 5 ledgers")
			assert.Contains(t, output.String(), tc.want+" read the same fixed range")
			assert.Equal(t, strings.ReplaceAll(tc.want, " and ", ","), p.Extra["fixedReadRange"])
		})
	}
}
...
func TestQueryLegWarningsUseSuccessfulSubgroups(t *testing.T) {
	...
	logQueryLeg(logger, queryTypeTxHash, 200, 0.5, res)
	...
	// A miss fraction the schedule cannot turn into 100 misses keeps that
	// subgroup quiet; the total row still warns.
	output.Reset()
	logQueryLeg(logger, queryTypeTxHash, 200, 0.1, res)
	assert.Contains(t, output.String(), "total_r200 has 0 successful samples")
	assert.Contains(t, output.String(), "found_r200 has 0 successful samples")
	assert.NotContains(t, output.String(), "miss_r200 has")
}
