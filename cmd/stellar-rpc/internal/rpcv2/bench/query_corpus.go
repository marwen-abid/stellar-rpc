...
// minEventScanPerChunk floors the per-chunk share of eventScanCap, so a
// many-chunk range still reads enough events from each chunk to rank its terms.
...
// and checks that one of them resolves under the passphrase. target must be in
...
func buildTxHashCorpus(
	...
	for i, c := range f.Chunks {
		// Each chunk fills the pool to its cumulative share, so a chunk that
		// comes up short is made up by the next one. With more chunks than
		// hashes some chunks necessarily contribute none.
		n := int64(len(f.Chunks))
		s.target = int((int64(target)*int64(i+1) + n - 1) / n)
		if len(s.hashes) >= s.target {
			continue
		}
		// A view holds every reader it opened until Release, so each chunk gets
		// its own: one view over the whole loop keeps every chunk's ledger reader
		// open and its file descriptor and offsets in the run's peak RSS.
		err := func() error {
			view, err := f.view()
			if err != nil {
			...
			defer view.Release()
			return s.sampleChunk(view, c, f.FirstLedger, f.LastLedger)
		}()
		if err != nil {
			return nil, err
		}
	...
	err := func() error {
		...
		defer view.Release()
		return verifySampledHashResolves(ctx, view, f, hash, seq)
	}()
	if err != nil {
		return nil, err
	}
...
func buildEventFilterCorpus(
	...
	contracts, pairs, err := scanEventTerms(ctx, f)
	if err != nil {
		return nil, err
	}
	...
	if len(sets) == 1 {
		logger.Warnf("events corpus holds only the unfiltered read: no contract ID or " +
			"(contract, topic) term was found in the scanned events; every events " +
			"request measures the same unfiltered page")
	}
	return &eventFilterCorpus{sets: sets}, nil
}

// corpusKind is what invocation.json records under eventsCorpus: "derived"
// when the scan found a filter term, "unfiltered" when the only set is the
// unfiltered read.
func corpusKind(sets int) string {
	if sets > 1 {
		return "derived"
	}
	return "unfiltered"
...
// scanEventTerms reads up to eventScanCap stored events across the fixture's
// chunks and returns the contract IDs and the (contract, first topic) pairs by
// descending frequency, as the store's canonical term bytes. Each chunk is read
// under its own view.
func scanEventTerms(ctx context.Context, f *queryFixture) ([][]byte, []eventTermPair, error) {
	if len(f.Chunks) == 0 {
		return nil, nil, nil
	}
	...
	// Every scanned chunk gets an even share of the budget, so the terms rank
	// over the whole range. A range holding more chunks than the cap can give
	// minEventScanPerChunk to is strided, so the scanned chunks still spread
	// from the first to the last. A chunk holding fewer events than its share
	// scans all of them; the remainder is not reassigned, so a range of short
	// chunks reads less than the cap.
	stride := max((len(f.Chunks)*minEventScanPerChunk+eventScanCap-1)/eventScanCap, 1)
	perChunk := max(eventScanCap/((len(f.Chunks)+stride-1)/stride), minEventScanPerChunk)
	for i := 0; i < len(f.Chunks); i += stride {
		if err := counts.scanChunk(ctx, f, f.Chunks[i], counts.scanned+perChunk); err != nil {
			return nil, nil, err
		}
	}
...
// scanChunk tallies chunk c's event terms under a read view of its own, and
// stops once the tally reaches limit events.
func (t *eventTermCounts) scanChunk(ctx context.Context, f *queryFixture, c chunk.ID, limit int) error {
	view, err := f.view()
	if err != nil {
		return fmt.Errorf("acquire read view: %w", err)
	}
