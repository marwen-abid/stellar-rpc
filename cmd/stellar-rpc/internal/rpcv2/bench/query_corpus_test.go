...
func TestCorpusTakesEveryHashOfALedgerBelowTheCap(t *testing.T) {
	...
	require.Zero(t, corpusTargetHashes%txPerLedger,
		"the target must divide evenly or the last ledger is trimmed")
...
func TestCorpusSkipsLedgersWithoutTransactions(t *testing.T) {
	// Short and dense enough that the bounded draw budget reaches every
	// tx-bearing ledger. The stride is test-local because eventEvery is sized
	// for the cold-ingest fixtures.
	const sparseTxEvery = 10
	const numLedgers = 2 * sparseTxEvery
	const txLedgers = numLedgers / sparseTxEvery
	first := chunk.ID(0).FirstLedger()
	packDir := writeLedgerPack(t, t.TempDir(), chunk.ID(0), numLedgers, func(seq uint32) []byte {
		if (seq-first)%sparseTxEvery == 0 {
	...
	assert.Len(t, s.ledgers, txLedgers, "every tx-bearing ledger must contribute")
	assert.Len(t, s.hashes, len(s.ledgers), "each of these ledgers holds one transaction")
	for _, seq := range s.ledgers {
		assert.Zero(t, (seq-f.FirstLedger)%sparseTxEvery, "ledger %d carries no transaction", seq)
	}
}
...
func buildCorpusCapturingLogs(t *testing.T, f *queryFixture) (string, []string) {
	...
	_, err := buildTxHashCorpus(context.Background(), logger, f, 0.1, defaultSeed, corpusTargetHashes)
	entries := done()
	require.NoError(t, err)
...
func TestBuildTxHashCorpusExactTarget(t *testing.T) {
	...
	for _, target := range []int{1, 7, 17, corpusTargetHashes, 513, 1025} {
		t.Run(fmt.Sprintf("target=%d", target), func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Len(t, first.hashes, target, "the last ledger must be trimmed to the exact target")
			assert.Len(t, uniqueHashes(first.hashes), target)
			...
			assert.Equal(t, (target+corpusMaxHashesPerLedger-1)/corpusMaxHashesPerLedger, first.ledgerCount)
			for seq, count := range counts {
				assert.LessOrEqual(t, count, corpusMaxHashesPerLedger, "ledger %d", seq)
			}
...
// TestBuildTxHashCorpusWithoutTransactions covers a range whose ledgers hold no
// transactions at all, which leaves the sampler's pool empty.
func TestBuildTxHashCorpusWithoutTransactions(t *testing.T) {
	const numLedgers = 4
	packDir := writeLedgerPack(t, t.TempDir(), chunk.ID(0), numLedgers, func(seq uint32) []byte {
		return rpcv2test.ZeroTxLCMBytes(t, seq)
	})
	...
	_, err := buildTxHashCorpus(context.Background(), testLogger(), f, 0, defaultSeed, corpusTargetHashes)
