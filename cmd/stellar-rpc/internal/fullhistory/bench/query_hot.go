...
type hotQueryOptions struct {
	...
	// dir. The DB is opened read-only.
	HotRoot string
...
// per-chunk hot RocksDB, opened read-only and shared by all workers — the
// live serving shape. Each sweep cell runs --warmup untimed iterations
// per worker first so the block cache reaches steady state.
func runQueryHot(ctx context.Context, logger *supportlog.Entry, opts hotQueryOptions) error {
	...
	db, err := hotchunk.OpenReadOnly(dbPath, opts.Chunk, logger)
	if err != nil {
		return fmt.Errorf("open hot DB %s read-only: %w", dbPath, err)
	}
...
// order against the shared read-only DB, preparing untimed corpus/preflight
