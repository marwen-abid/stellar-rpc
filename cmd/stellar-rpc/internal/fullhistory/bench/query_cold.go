...
func runQueryCold(ctx context.Context, logger *supportlog.Entry, opts coldQueryOptions) error {
	...
	if err := runColdTypes(ctx, logger, geometry.NewLayout(opts.ColdRoot), opts, sink); err != nil {
...
func runColdPackTypes(
	...
	if opts.Types.Txhash {
		...
		err = runSweep(ctx, sink, opts.sweepSpecFor(ingest.DataTypeTxhash), op)
		cleanup()
		if err != nil {
			return err
		}
...
func inspectColdPacks(layout geometry.Layout, chunks []chunk.ID) ([]coldPack, error) {
	...
	for _, c := range chunks {
		...
		first, ferr := cr.FirstSeq()
		last, lerr := cr.LastSeq()
		if err := errors.Join(ferr, lerr, cr.Close()); err != nil {
			return nil, fmt.Errorf("ledger pack %s: %w", path, err)
		}
		packs = append(packs, coldPack{id: c, path: path, first: first, last: last})
	}
	return packs, nil
}
...
func resolveTxhashIndex(
	...
	idxPath := filepath.Join(tmpDir, txhash.IndexFileName(packs[0].id))
