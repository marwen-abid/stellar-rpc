...
// it, and benchmark all four query types against it.
...
// requirePeakRSSRow asserts the always-on peak_rss_bytes driver row. The row
// exists only on Linux (readPeakRSS reads procfs), so on other platforms this
// asserts nothing.
func requirePeakRSSRow(t *testing.T, driver map[string]map[string]int64) {
	t.Helper()
