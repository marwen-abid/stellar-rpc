...
func newQueryColdCommand() *cobra.Command {
	...
	fs.Uint32Var(&chunkArg, "chunk", 0, "first chunk ID to benchmark (required)")
	...
	markRequired(cmd, "types", "chunk", "cold-dir")
	return cmd
}
...
func newQueryHotCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "hot",
		Short: "Benchmark hot queries against one chunk's hot RocksDB, opened read-only",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			cmd.SilenceUsage = true
			knobs, err := flags.knobs()
