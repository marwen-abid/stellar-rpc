...
// basename). The ingest engine does not emit this label, so it has
// no coldType* constant.
...
func registerQueryReport(sink *csvSink, k queryKnobs) {
	...
	for _, t := range []struct {
		...
		{name: coldTypeLedgers, on: k.Types.Ledgers},
		{name: typeTxPage, on: k.Types.TxPage, setup: []string{driverPagePreflight}},
		{name: coldTypeTxhash, on: k.Types.Txhash, setup: []string{driverHashSample, driverIndexBuild}},
		{name: coldTypeEvents, on: k.Types.Events, setup: []string{driverCorpusScan}},
	} {
		if !t.on {
			continue
		}
