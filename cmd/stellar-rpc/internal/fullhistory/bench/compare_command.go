...
// candidate build, judges each gated metric's per-side means with an exact
// permutation test (Bonferroni-corrected across the metrics) plus min-effect
// and noise gates, and exits non-zero naming every regressed metric. See
// compare.go for the statistics.
