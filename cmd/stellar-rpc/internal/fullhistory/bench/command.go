...
func newColdCommand() *cobra.Command {
	...
	fs := cmd.Flags()
	fs.Uint32Var(&chunkArg, "chunk", 0, "first chunk ID to backfill (required)")
...
func newHotCommand() *cobra.Command {
	...
	fs.IntVar(&numChunks, "num-chunks", 1,
		"how many consecutive chunks to ingest starting at --chunk (>1 exercises the hot DB rotation)")
