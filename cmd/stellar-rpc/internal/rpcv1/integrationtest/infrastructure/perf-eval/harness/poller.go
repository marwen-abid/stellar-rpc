...
func (p *resultPoller) poll(ctx context.Context, until time.Time) (*Result, error) {
	...
	for pollCount := 1; time.Now().Before(until); pollCount++ {
		...
		switch {
		// Every poller key is pre-seeded with a pending marker by its workflow
		// (the leg job or the campaign launch), so an absent object is a
		// seeding or config fault and counts. In practice the role has no
		// s3:ListBucket, which turns a missing key into a 403 that the generic
		// error path below catches; this path covers a lifecycle deletion and
		// a bucket that does grant the listing. checkOnce already logged it.
