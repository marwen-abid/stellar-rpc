# Ingestion-p99-vs-target recap from the converted run JSON. Per profile: p99 = driver.ingest_total.p99
# aggregated median (ns); the name is the phase workload with the same tx/ledger the unit name encodes.
# Tiers: over the block time, over the phase target, under it. null (no phase/target/block/hot ingest) drops the block.
