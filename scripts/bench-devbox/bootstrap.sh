#!/usr/bin/env bash
#
# Idempotent bootstrap for the full-history benchmark dev box (user-dev-063a,
# m6id.2xlarge, Ubuntu 24.04 SDF base AMI). Safe to re-run any time — in
# particular after an instance stop/start, which wipes the NVMe instance
# store (golden packs are re-downloaded by campaign.sh in ~3 minutes).
#
# Usage (on the box):
#   ./scripts/bench-devbox/bootstrap.sh
#
# Overridable: NVME_DEV (default /dev/nvme1n1), REPO (default ~/stellar-rpc),
# BRANCH (default bench-ci-775).
#
set -euo pipefail
...
if ! /usr/local/go/bin/go version 2>/dev/null | grep -Eq 'go1\.(2[6-9]|[3-9][0-9])'; then
  ...
  # decompress as the user: sudo'd tar cannot exec gzip on this AMI
  gunzip -f /tmp/go.tgz
...
if [ ! -d "$REPO/.git" ]; then
  note "cloning $BRANCH"
  git clone --branch "$BRANCH" https://github.com/marwen-abid/stellar-rpc.git "$REPO"
fi
